...
class WaypointTrackingController:
    ...
    def compute_command(self, state: RobotSystemState) -> RobotSystemCommand:
        ...
        if not scheduler_paused and self.advance_enabled:
...
class WipingController:
    ...
    def compute_command(self, state: RobotSystemState) -> RobotSystemCommand:
        ...
        corrected_waypoint = original_waypoint.copy()
        desired_velocity_override = None
        if (
            self.control_type == "contact_triggered_admittance"
            and self._admittance is not None
        ):
            self._tracking.scheduler.active_index = self._admittance.target_index
            waypoint_index = self._tracking.active_index
            target_force = float(self.target_force_n[waypoint_index])
            admittance_step = self._admittance.step(
                tip_position=executor.tip_pose_world.position,
                target_positions=self._tracking.waypoints_world,
                normal=(
                    query.normal
                    if query is not None
                    else self.surface_normal_world
                ),
                measured_normal_force_n=(
                    0.0 if not np.isfinite(estimated_force) else estimated_force
                ),
                dt=self.controller_dt_s,
            )
            self._tracking.scheduler.active_index = admittance_step.target_index
            waypoint_index = self._tracking.active_index
            original_waypoint = self._tracking.waypoints_world[waypoint_index].copy()
            corrected_waypoint = admittance_step.corrected_target_position
            desired_velocity_override = admittance_step.desired_velocity
        elif self.phase == "contact" and np.isfinite(contact_error) and query is not None:
            ...
            corrected_waypoint = original_waypoint + normal_correction * query.normal
        if self.phases:
            self.phase = self.phases[waypoint_index]
        target_force = float(self.target_force_n[waypoint_index])
        force_error = (
            float("nan")
            if not np.isfinite(estimated_force)
        ...
        self._tracking.waypoints_world[waypoint_index] = corrected_waypoint
        self._tracking.executor_velocity_override_world = desired_velocity_override
        try:
            command = self._tracking.compute_command(state)
        finally:
        ...
        if self.control_type == "dynamic_adaptive_impedance":
            command = self._with_dynamic_executor_command(
                state,
                command,
                corrected_waypoint,
                estimated_force,
                query.normal if query is not None else self.surface_normal_world,
            )
            dynamic_active = True
        admittance_command = (
            None if self._admittance is None else self._admittance.last_command
        )
        return RobotSystemCommand(
            base_twist_world=command.base_twist_world,
            arms=command.arms,
            metadata={
                **command.metadata,
                ...
                "wiping_dynamic_system_controller_active": dynamic_active,
                "wiping_admittance_active": self.control_type == "contact_triggered_admittance",
                "wiping_admittance_position_m": (
                    np.nan
                    if admittance_command is None
                    else admittance_command.admittance_position_m
                ),
                "wiping_admittance_velocity_m_s": (
                    np.nan
                    if admittance_command is None
                    else admittance_command.admittance_velocity_m_s
                ),
