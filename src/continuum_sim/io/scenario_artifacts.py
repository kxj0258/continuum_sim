...
from continuum_sim.runtime.metadata_schema import (
    OBSERVER_SCALAR_METADATA,
    OBSERVER_VECTOR2_METADATA,
    OBSERVER_VECTOR3_METADATA,
    ORIENTATION_VECTOR4_METADATA,
)
...
def _flatten_result(application, result) -> dict[str, np.ndarray]:
    ...
    if result.commands:
        ...
        for key in OBSERVER_VECTOR2_METADATA:
            arrays[key] = np.asarray(
                [
        ...
        for key in ORIENTATION_VECTOR4_METADATA:
            arrays[key] = np.asarray(
                [
