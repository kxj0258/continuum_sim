...
曲率控制程序同时管理执行臂 `executor` 和观测臂 `observer`。每一段的局部 `kx/ky` 均提供滑块和数值输入框，范围固定为 `-30～+30 1/m`：拖动滑块可连续调节，输入数值并按回车键可精确设置目标。越界输入会被限制到有效范围，非法输入会恢复为当前有效值。曲率目标不执行世界坐标或安装坐标逆解。

拉线控制程序为每条臂提供九根肌腱目标滑块和数值输入框，并固定使用原始拉线调试模式。两个程序都提供基座 `X/Y/Z/Roll/Pitch/Yaw` 调节和独立状态窗口。状态窗口实时显示：
...
曲率控件回调只发布最新目标，不直接回写其他控件。滑块和数值输入框在低频界面刷新阶段统一同步，并且只处理发生变化的曲率分量，以免界面重绘影响独立的 `50 Hz` 控制线程。旧的曲率增减按钮和 `--curvature-step` 参数已删除。
