...
class MujocoObserverCameraFeedbackHook:
    ...
    def enrich_state(self, state: RobotSystemState) -> RobotSystemState:
        ...
        if target is None or self._mujoco is None:
            return state
        try:
            ...
            metadata = {**state.metadata, **feedback.as_metadata()}
            metadata["visual_servo_camera_name"] = self.camera_name
            if self.errors:
                metadata["visual_servo_camera_errors"] = tuple(self.errors)
            if self.show_window and self._frame_index % self.stride == 0:
                self._render_window(data)
            self._frame_index += 1
